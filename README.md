# Glassmorphism Login UI Concept

A lightweight, customizable login card interface built with HTML5 and CSS3. Designed as a clean starter template or layout concept that you can easily tweak, expand, or build onto for your own web projects.

## Features

* **Glassmorphism Styling:** Blurred, translucent card background and input fields using `backdrop-filter: blur(10px)`.
* **Hover Micro-Interactions:** Smooth scale transformations and color transitions on text inputs and the submit button.
* **Pure CSS Layout:** Fully centered full-screen display powered by Flexbox without any JavaScript dependencies.
* **Proper Form Markup:** Native HTML `<form>` structure with text fields and a functional submit button ready for backend 

## Project Structure

* `index.html` - Form structure, inputs, submit button
* `style.css` - Glassmorphism rules, transitions, positioning, and hover states
* `esaias-tan-WmpCzw4QXLA-unsplash.jpg` - Background photo asset

## Author

* **cfh0714**
* Background photo by [Esaias Tan](https://unsplash.com/@esaiast) on Unsplash
